...
st.set_page_config(page_title="포켓몬 도감", page_icon="./images/monsterball.png")
...
st.subheader("asdasdasd포켓몬을 하나씩 추가해서 도감을 채워보세요~!")
...
with st.form(key="form"):
    ...
    with col1:
        name = st.text_input(
            label="포켓몬 이름", value=example_pokemon["name"] if auto_complete else ""
        )
    with col2:
        ...
        with cols[j]:
            ...
            with st.expander(label=f"**{i+j+1}. {pokemon["name"]}**", expanded=True):
                ...
                delete_button = st.button(
                    label="삭제", key=i + j, use_container_width=True
                )
                if delete_button:
